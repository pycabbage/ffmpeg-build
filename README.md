...
ビルド環境は **すべてソースからビルド**する（`docs/todo/001-full-build.md`）。ツールチェイン（gcc/binutils + 依存）、ビルドツール（cmake/meson/ninja/nasm/python など）、全メディアライブラリを `/usr/local` にソースビルドし、apt はライブラリを一切 install しない（OS フロア + 自前 gcc 用ブートストラップ seed のみ）。**patchelf は使わず**、自前 gcc の `specs` でリンク時に `$ORIGIN` RPATH を焼き込んで再配置可能にする。
...
- `out/lib/` — バンドルされた共有ライブラリ（`libavcodec.so.*`、コーデック / フィルタ libs、`libstdc++` など）。各バイナリは **リンク時に焼き込まれた** RPATH `$ORIGIN/lib`（自前 gcc の specs による。patchelf 不使用）を持つため、`out/lib/` から自分のライブラリを解決する（イメージ内パスに依存しない）。
