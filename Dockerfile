...
# clang/LLVM (from source) for FFmpeg --enable-cuda-llvm. Built BEFORE libplacebo (the last,
# still-iterated media lib) so editing libplacebo never cache-busts this heavy ~30-90min layer.
# Build-time tool only — not linked into the bundle; needs only the phase-2 toolchain.
RUN --mount=type=bind,source=scripts/deps/common.sh,target=/opt/scripts/deps/common.sh \
