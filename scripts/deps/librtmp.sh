...
# No versioned releases; project lives at git.ffmpeg.org/rtmpdump. We pin a specific COMMIT
# (checked out below) for reproducibility instead of tracking a moving master. Build only the
...
# rtmpdump has no tags, so fetch_git (--branch) cannot pin a SHA; clone then detach to COMMIT.
rm -rf "${SRC}"
git clone "https://git.ffmpeg.org/rtmpdump.git" "${SRC}"
